...
export default function IAPItemEditPage() {
  ...
  const fetchItem = async () => {
    ...
    try {
      const response = await fetch(`${BASE_URL}/api/v1/iap-items/${itemId}`);
      if (!response.ok) throw new Error("IAP 아이템 정보를 불러올 수 없습니다.");
      ...
      setFormData({
        ...
        purchaseType: item.purchaseType || "PAID",
      ...
      setOriginalImageUrl(item.imageUrl || null);
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      const isOriginal = originalImageUrl === oldImageUrl;
  ...
  const handleImageDelete = async () => {
    ...
    const isOriginal = originalImageUrl === formData.imageUrl;
  ...
  const handleDelete = async () => {
    if (!confirm("정말로 이 IAP 아이템을 삭제하시겠습니까?")) return;
    ...
    try {
      ...
      if (!response.ok) throw new Error("IAP 아이템 삭제에 실패했습니다.");

      alert("IAP 아이템이 삭제되었습니다.");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error?.message || "IAP 아이템 수정에 실패했습니다.");
      }

      alert("IAP 아이템이 성공적으로 수정되었습니다!");
      router.push("/sky/SeasonDictionary");
    } catch (err) {
      setError(err.message);
  ...
  return (
    <div className={styles.container}>
      <div className={styles.header}>
        <h1 className={styles.title}>IAP 아이템 수정</h1>
